...
function auth_login($login, $password, $db_connection)
{
    $auth_result = null;
    unset($_SESSION['user_id']);
    if ($db_connection) {
        $escaped_login = $db_connection->real_escape_string($login);
        $escaped_password = $db_connection->real_escape_string($password);
        $query = "SELECT id, name, surname, age FROM user WHERE LOWER(login)=LOWER('{$escaped_login}') and LOWER(password_hash) = LOWER('$escaped_password')";

        if ($result = $db_connection->query($query)) {
            if ($row = $result->fetch_row()) {
                $_SESSION['user_id'] = $row[0];
                $_SESSION['user_name'] = $row[1];
                $_SESSION['user_surname'] = $row[2];
                $_SESSION['user_age'] = $row[3];
            } else {
...
{
    return $_SESSION['user_name'] . ' ' . $_SESSION['user_surname'];
}

function get_user_age()
{
    switch (substr($_SESSION['user_age'], -1)) {
        case 1 :
        {
            $age_suffix = 'год';
            break;
    ...
    return $_SESSION['user_age'] . ' ' . $age_suffix;
}
...
switch (strtolower($_POST['auth_action'])) {
    case 'login' :
    {
        $auth_result = auth_login($_POST['login'], $_POST['md5password'], $db_connection);
...
<main>
    <form action="" method="post" onsubmit="pwd_calc(this);">
        ...
        <?php if (is_auth()): ?>
            <div class="form-item"><input type="text" readonly value=" <?php echo get_user_fullname() ?> "></div>
            <div class="form-item"><input type="text" readonly value=" <?php echo get_user_age() ?> "></div>
            <div class="form-item">
                <button type="submit" name="auth_action" value="exit">выход</button>
            </div>
        <?php else: ?>
            <div class="form-item"><input type="text" name="login" placeholder="login (tom,bob)" autocomplete="off"></div>
            <div class="form-item"><input type="password" name="password" placeholder="password (111,222)"></div>
